...
export async function load({ params, url }) {
  ...
  try {
    ...
    if (params.essay || url.pathname.includes("data")) {
      // console.log(params.essay);
      // console.log(url.pathname.includes("data"));
      if (url.pathname.includes("data")) {
        const dataaa = switchGitter("all");
        // return dataaa;
        let minis = [];
        for (const path in dataaa) {
          // console.log(path);
          const file = dataaa[path];
          if (file && typeof file === "object" && "metadata" in file) {
            // console.log("asdfasdf");
            minis.push(dataaa[path].metadata);
          }
        }
        return { minis };
      }
